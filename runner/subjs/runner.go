...
type SubJS struct {
	...
	// sem bounds the number of HTTP requests in flight at any moment,
	// across BOTH top-level input URLs and every recursively-discovered
	// JS file, to -c (opts.Workers). This replaces the old model where a
	// fixed pool of -c goroutines only ever pulled top-level input URLs
	// off a channel, while every recursive fetch (script -> discovered
	// ref -> discovered ref...) was called directly/synchronously inside
	// whatever goroutine found it - meaning a single input URL with a
	// long reference chain (e.g. a heavily-chunked webpack bundle) was
	// fetched one hop at a time in one goroutine, regardless of -c. Over
	// a slow transport like Tor this serializes total crawl time to
	// (chain length) * (per-request latency), and since there's no
	// retry, a single slow/failed hop silently truncates everything
	// beneath it - which looks like "-depth doesn't reach as deep" or
	// "raising -t finds more URLs", when what's actually happening is
	// that recursion was never parallelized at all. See spawn() below.
	sem chan struct{}

	// pending tracks every in-flight fetch, top-level or recursive, so
	// Run() knows when the entire - dynamically growing - reference tree
	// has finished, not just the initial batch of input URLs.
...
func New(opts *Options) *SubJS {
	...
	return &SubJS{client: c, opts: opts, scopeRules: buildScopeRules(opts.Scope), sem: make(chan struct{}, opts.Workers)}
}

// spawn runs fn in its own goroutine, gated by the shared semaphore (which
// caps total concurrent HTTP requests at -c, exactly as before - just
// applied uniformly now instead of only to top-level input URLs) and
// tracked by the shared pending WaitGroup (so Run can tell when every
// fetch, at any recursion depth, has actually finished). Calling spawn
// itself never blocks the caller - only the goroutine it starts blocks,
// on the semaphore - so it's always safe to call from inside another
// spawned fetch without risking deadlock.
func (s *SubJS) spawn(fn func()) {
	s.pending.Add(1)
	go func() {
...
func (s *SubJS) Run() error {
	...
	// seen is the shared dedup map across all in-flight fetches and all
	// recursion depths - see report() above.
	seen := &sync.Map{}
	...
	for scan.Scan() {
		...
		if u != "" {
			...
			seen.Store(u, true)
			u := u // capture for the closure below
			s.spawn(func() {
				s.fetchOne(u, results, seen)
			})
		}
	}

	// Every input URL, and every fetch recursively spawned from it at any
	// depth, is tracked by s.pending (via spawn) - so this waits for the
	// entire dynamically-discovered reference tree to finish, not just
	// the initial batch of input URLs.
	s.pending.Wait()
	close(results)
	out.Wait()
	return nil
...
// fetchOne fetches and processes a single URL: an input URL, handed to us
// via spawn() from Run(). Recursively-discovered JS files are also
// processed via fetchAndScanJS, which - like every other call site that
// finds a new URL - dispatches its own recursive work through spawn()
// rather than fetching it inline, so sibling references at any depth are
// fetched concurrently (bounded by -c) instead of one at a time. See the
// SubJS.sem/pending field docs above for why this matters.
func (s *SubJS) fetchOne(u string, results chan string, seen *sync.Map) {
	...
	// A non-200 status is very commonly a block/challenge page (rate
	// limiting, WAF, or - especially over Tor - a CDN simply refusing
	// exit-node traffic) rather than the real asset, but plenty of
	// sites also serve a perfectly real, JS-referencing body alongside
	// an error code (e.g. a 403 page that still ships the site's normal
	// <script> tags, or a custom error page reusing the main layout).
	// So the body is scanned regardless of status code - only logged
	// via -debug when it isn't exactly 200, so a block/challenge page
	// is still visible as a likely explanation for unexpected output.
	if statusCode != 200 {
		s.debugf("%s: HTTP %d, scanning anyway", u, statusCode)
	}
	...
	doc.Find("script").Each(func(index int, scriptTag *goquery.Selection) {
		...
		if exists && js != "" {
			...
			if s.report(resolvedJS, results, seen) {
				...
				// -depth further hops. Skipped entirely under
				// -nodepth (Depth == -1): that mode only reports the
				// JS URLs directly visible on the input page itself
				// and never fetches anything discovered from it.
				// Dispatched via spawn() so this (and every sibling
				// <script>/<link> on the page) is fetched concurrently
				// instead of blocking here one at a time - see the
				// SubJS.sem/pending docs.
				if s.opts.Depth >= 0 {
					depth := s.opts.Depth
					s.spawn(func() { s.fetchAndScanJS(resolvedJS, results, seen, depth) })
					s.spawn(func() { s.checkSourceMap(resolvedJS, results, seen) })
				}
			}
		}
	...
	doc.Find("link").Each(func(index int, linkTag *goquery.Selection) {
		...
		if s.report(resolvedJS, results, seen) {
			htmlFound = true
			// See the <script src> case above: skipped entirely
			// under -nodepth (Depth == -1); dispatched via spawn()
...
//
// Unlike the top-level fetchOne() (which handles whatever content type a
...
//
// This function itself runs synchronously (one HTTP request, then scans
// the body inline) - it's the *caller* (see scanForJSReferences/
// ProcessWebpackFile/checkLinkHeaderForScripts/fetchOne) that dispatches
// each call via spawn() rather than calling it inline, so that sibling
// references discovered at the same depth are fetched concurrently
// (bounded by -c) instead of one at a time.
func (s *SubJS) fetchAndScanJS(jsURL string, results chan string, seen *sync.Map, depth int) {
	...
	if statusCode != 200 {
		s.debugf("%s: HTTP %d, scanning anyway", jsURL, statusCode)
	}
...
// fetchOne() would otherwise miss entirely. Uses the raw header map instead
...
func (s *SubJS) checkLinkHeaderForScripts(header http.Header, baseURL *url.URL, results chan string, seen *sync.Map, depth int) {
	for _, headerValue := range header[http.CanonicalHeaderKey("Link")] {
		for _, entry := range linkHeaderEntryRe.FindAllStringSubmatch(headerValue, -1) {
			...
			if s.report(resolvedJS, results, seen) {
				// See the <script src> case in fetchOne(): skipped
				// entirely under -nodepth (depth == -1); dispatched via
				// spawn() for the same concurrency reason.
				if depth >= 0 {
					d := depth
					s.spawn(func() { s.fetchAndScanJS(resolvedJS, results, seen, d) })
					s.spawn(func() { s.checkSourceMap(resolvedJS, results, seen) })
				}
			}
		}
...
func (s *SubJS) scanForJSReferences(baseURL *url.URL, content string, results chan string, seen *sync.Map, depth int) bool {
	...
	handle := func(js string) {
		...
		if s.report(resolved, results, seen) {
			...
			if depth > 0 {
				d := depth - 1
				s.spawn(func() { s.fetchAndScanJS(resolved, results, seen, d) })
				s.spawn(func() { s.checkSourceMap(resolved, results, seen) })
			}
		}
	}
...
func (s *SubJS) ProcessWebpackFile(webpackURL string, content string, results chan string, seen *sync.Map, depth int) bool {
	...
	emit := func(resolvedURL string) {
		if s.report(resolvedURL, results, seen) {
			...
			if depth > 0 {
				d := depth - 1
				s.spawn(func() { s.fetchAndScanJS(resolvedURL, results, seen, d) })
				s.spawn(func() { s.checkSourceMap(resolvedURL, results, seen) })
			}
		}
	}
